...
# abstract: this script is used to convert csv files from MOForest into .dbs files so that they can be imported into MOFITS
...
while not done:
    ...
    if ext == '.csv':
        ...
        outfile = str(os.path.join(dirname, file_name)) + ".dbs"
        with open(infile, 'rb') as reader, open(outfile, 'wb') as writer:
            for line in reader:
                newline = line.strip('\r\n') + ',0,,'
                writer.write(newline)
